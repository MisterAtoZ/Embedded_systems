...
void __attribute__((interrupt("IRQ"))) interrupt_vector(void)
{
    ...
    *************************************************************/
   volatile uint32_t IRQ_pending_2_register = RPI_GetIrqController()->IRQ_pending_2; //register uithalen (pending 2 register)
   if((IRQ_pending_2_register & RPI_IRQ_2_ARM_UART_IRQ) >> 25)  {
       uart_handler(&uart);
   }
}
